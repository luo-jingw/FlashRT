...
- Wiring: candidates 1, 3, 4, 6, 8 have standalone kernels/designs ready; none is wired into `pipeline_thor.py`, `csrc/bindings.cpp` or `CMakeLists.txt`. Wiring is inherently serial (1, 3, 8 all touch the same per-layer functions in `pipeline_thor.py`), one owner, one fusion at a time, each gated by `imagewam_fusion_ab.py`-style A/B, following OPT-015/016/017's own precedent (and each behind its own `dims` flag so it stays toggleable, matching `merge_qkv_mlp`/`merge_linear2`/`fuse_res_norm`). Not started.
- Before wiring 3: directly test the new kernel against TODAY's `gate_res_ada_layer_norm_kernel` (fusion.cu, `fuse_res_norm=True`), not only the unfused pair the authoring agent validated against -- the transitive bit-exactness argument above is strong but unconfirmed.

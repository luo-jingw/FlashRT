...
The Thor latency baseline is `nvfp4` 202.2 ms, margin 5%, re-based from 231.6 ms, and describes the untrimmed, FA4-off configuration; H100 latency is ungated.
...
| per-precision fidelity thresholds, per-device latency policy (Thor `nvfp4` 202.2 ms, margin 5%; H100 ungated) | `tests/fixtures/imagewam_gate/fidelity_thresholds.json`, `tests/fixtures/imagewam_gate/latency_baselines.json` |
...
- The latency baseline (`latency_baselines.json`, `nvfp4` 202.2 ms, margin 0.05) has no per-configuration record: it describes the untrimmed, FA4-off configuration as a one-sided bound and is applied to both configurations the gate can be asked for. The served default (trim + FA4) measures 125.86 ms in the same gate, so the bound 202.2 x 1.05 = 212.3 ms fires only after a regression of about 69%, while the untrimmed row it was recorded from measures 191.79 ms in the `0920t` session, about 11%. Re-seeding it per configuration, and stating the configuration each number describes (trim on or off, FA4 state), is an open owner decision (plan.md "Open" item 1; issues.md ISSUE-061).

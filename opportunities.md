...
- FA4 at the `mot` site: promoted to the served default on 0921 (owner decision, `plan.md` phase F1) on the `c20f3a0` ladder's evidence, and confirmed by the `0921` Thor round (OPT-030): the fixture-v2 gate of the new default keeps vs official at 0.99931-0.99933 with no fallback, and the `mot` site's own marginal is -4.5 ms (`vae_trim_fa4bb` 98.9 to `stack` 94.4).
...
Status: candidates 1, 3, 4, 6 and 8 have standalone, bit-exact-tested kernels (local Ada, `CUDA_VISIBLE_DEVICES=0`, RTX 4060 sm_89, not wired into `pipeline_thor.py`/the main build); 2, 5, 7, 9-12 are still only analysis. The Thor stage x operator grid (`benchmarks/imagewam_stage_operator_grid.py`, THOR_CHECKLIST.md X5) is still the measurement that confirms which of these actually move the ~5350-node graph on Thor; wiring is unstarted. Sizes marked (inf) are inferences, not measurements; local Ada timings are labelled and never predict Thor.

**Process note**: the four kernel-authoring subagents for this round ran with `isolation: "worktree"`, which branches from `origin/main` (then at `008918e`, far behind `roadmap/integration`), not the current branch — none of them saw `libero_dims.py`, the `fuse_res_norm`/`gate_res_ada_layer_norm_kernel` cross-layer fusion (OPT-017, added in `afe00af`), or `merge_linear2` (`d2dd950`), all added after `008918e`. Checked before accepting their work: `csrc/kernels/norm.cu`, `rope.cu` and `common.cuh` are byte-identical between `008918e` and current HEAD (0 diff); `decoder_fused.cu` and `fusion.cu` only gained insertions (0 deletions) over the same range, and the unfused `gate_res_bf16res`/`ada_layer_norm_*` pair those agents validated against is still live code today (the `fuse_res_norm=False` path); `attn_backend.py`'s per-layer K/V bijection (`get_slot_ptrs`) is unchanged in shape despite 74 added lines. So the delivered kernels' numerics hold against current HEAD; only the WIRING (call sites, current buffer/flag names) needs a fresh read of the current `pipeline_thor.py`, not the subagents' line-number citations. `isolation: "worktree"` should not be used for kernel work on this repo without also passing the current branch, or without this same staleness check afterward.
...
|---|---|---|---|---|
| 1 | one kernel for Q/K/V split + QK-RMSNorm + RoPE + write to Q_O / K_cache / V_cache (replaces 3 copies + 2 rms + 2 rope) | -1670 (1500 denoise, 170 prefill) | 8-15 ms (inf); local Ada single-call ratio 4.18x (1.101 -> 0.264 ms at rows=905, informative only) | **kernel authored and bit-exact** (`csrc/kernels/fused_qkv_norm_rope/`, `tests/test_fused_qkv_norm_rope_kernel.py`, 8/8 `torch.equal` at rows in {25, 417, 905, 64}, plain and merged-linear1 shapes). A real synchronization bug (missing `__syncthreads()` between the Q and K block-reductions sharing one scratch array) was caught by the bit-exact check, not by a cosine check -- fixed. Double-stream needs this kernel called TWICE (once per txt/img stream, each at its own row offset into the shared RoPE table and the shared Q_O/K_cache destination); proved bit-exact against the reference's separate-norm-then-joint-rope sequence. Single-stream and both ActionDiT blocks need one call each. Even HD is a precondition (vectorized 4-byte-aligned loads); real ImageWAM's HD=128 always satisfies it. **Wired** into `_single_stream_layer`/`_action_single_layer` behind `dims["fuse_qkv_norm_rope"]` (default off), plan.md "Plan: wire OPT-032's kernel-fusion candidates into the served pipeline" Phase 1. Thor found two real problems in the WIRING TEST (not the kernel or the wiring): a seed-reset-too-late bug (`0922`, fixed) and, after that fix, a real crash (`0922b`) narrowed down locally to a pre-existing `GemmRunner` fragility unrelated to this kernel (issues.md ISSUE-090: two calls to the SAME layer function against a shared `GemmRunner` with fresh weight tensors at the same shape corrupts something a later cuBLAS call trips over, reproduced with `fuse_qkv_norm_rope=False` on both calls too). Once the test builds weights once and replays instead of rebuilding per call, it is confirmed bit-exact locally for BOTH backbone (`merge_qkv_mlp` True and False) and ActionDiT, stable over 3 runs. Thor-confirmed (`0922d`, HEAD `e727f91`): `1 passed`, all three comparisons `torch.equal`/`bit_exact=True`, `max_abs=0`. Phase 1 closed. Phase 2 (double-stream wiring, `_double_stream_layer`/`_action_double_layer`) done and Thor-confirmed (`0922e`, HEAD `c5898a2`: `2 passed`, all five comparisons `torch.equal`/`bit_exact=True`, `max_abs=0`; plan.md's own Phase 2 entry has the full account, including a test-construction bug found and fixed along the way -- a swapped `Fp16Linear` n/k argument for ActionDiT's `proj.weight`, unrelated to this kernel). Phases 1-2 of candidate 1's wiring are closed; Phase 3 (candidate 8) and Phase 4 (candidate 3) remain. |
| 2 | FA4 writes straight into its consumer (linear2 input / proj input), dropping the copy-back and the concat copy | -495 | 2-4 ms | not attempted (FA4 does not run on Ada at all, so this needs Thor for any validation, unlike 1/3/4/6/8) |
| 3 | gated-residual + AdaLN kernel emits FP4 + scale factors directly (no quantize before qkv / mlp0 / linear1, no fp16 `modded` round trip) | -340 | 2-4 ms (inf) | **kernel authored and bit-exact** (`csrc/kernels/fused_norm_fp4/`, `tests/test_fused_norm_fp4_kernel.py`: run directly, not pytest-collectible -- `python tests/test_fused_norm_fp4_kernel.py`; 16/16 bit-exact, both BF16-residual (backbone) and FP16-residual (ActionDiT) variants, rows in {25, 417, 905, 64}, dim in {3072, 1024}). Validated against the UNFUSED `gate_res_bf16res`/`gate_res_fp16` + `ada_layer_norm_*` pair on this dev machine; **confirmed on Thor too** (`0921x`, THOR_CHECKLIST.md X6): the new kernel's `packed`/`sfa`/`residual` outputs are `torch.equal` against `gate_res_ada_layer_norm_bf16res`/`_fp16` (the CURRENTLY-WIRED `fuse_res_norm=True` fusion, `fusion.cu`) + `quantize_fp4_dynamic_sfa_fp16`, 16/16 shapes. Bit-exact against today's production fused path, not just the older unfused pair; safe to wire. AWQ `inv_s` fold: same kernel, one extra nullable per-channel argument (diagnostic-only test found a rounding-order mismatch in the TEST's own reference construction, not the kernel -- the real kernel rounds once, a from-scratch two-kernel reference double-rounds). **Wired, Round 1 scope** (plan.md Phase 4): `Nvfp4Linear.gemm_prequantized` (skip-quantize GEMM), `AdaLNTarget.lin` + `_awq_target` (always attaches the consuming GEMM object), `_fused_gate_res`'s opt-in `fp4_direct` dispatch, and `_single_stream_layer`'s own `merge_qkv_mlp=True` `linear1.weight` consumer behind `dims["fuse_res_norm_fp4"]`. Also wired into the real build: `CMakeLists.txt`'s `fp4_kernels_obj` sources and production `flash_rt_fp4` bindings (`csrc/fp4_bindings.cpp`) -- this kernel was JIT-only before this round. CPU-only dispatch/call-order test passes 9/9 (`tests/test_imagewam_fuse_res_norm_fp4_dispatch.py`). First real Thor run (`0922f`) found a genuine bug: `gate`/`scale`/`shift`/`inv_s` are `const __half*` in this kernel (unlike the older `gate_res_ada_layer_norm_bf16res`'s `const float*`), and the wiring passed FP32 pointers, producing an O(1e3)/NaN residual instead of the correct O(1) value -- root-caused and fixed (round to fresh FP16 tensors before the call, issues.md ISSUE-091), CPU dispatch suite re-verified 9/9. **Thor-confirmed** (`0922g`, HEAD `3056b03`, no rebuild needed): `1 passed`, the two-layer single-stream chain `torch.equal`/`bit_exact=True`, `max_abs=0`. Phase 4 Round 1 closed. Round 2: same pattern extended to `_action_single_layer` (ActionDiT's own single-stream chain); **Thor-confirmed** (`0922h`, HEAD `11e8869`): `3 passed`, all `torch.equal`/`bit_exact=True`, `max_abs=0` (X10's backbone chain re-confirmed too). Round 3 (closing the double->single boundary, `_double_stream_layer`/`_action_double_layer`) was ATTEMPTED and REVERTED this round after finding a genuine kernel limitation: `fused_norm_fp4.cu`'s `launch()` computes its CUTLASS tile-interleaved SFA layout fresh from `seq_len` every call (`tile_atom_to_shape_SFA(make_shape(seq_len, ...))`), with no row-offset/total-seq-len parameter -- unlike candidate 1's kernel, it cannot be called twice (once for `_double_stream_layer`'s txt rows, once for its img rows) to jointly populate one combined a0-row destination scratch the way `_single_stream_layer`'s consumer (`linear1.weight`, ONE GEMM over all a0 rows) needs; the two calls would each compute their own small tile layout at the same base pointer instead of two slices of one larger tiled layout. Closing this needs either a kernel change (an explicit row-offset parameter) or a consumer-side redesign (two separate GEMMs instead of Round 1's single combined one) -- real design work, not attempted, a Round 4. Neither Round 1 nor Round 2 covers this boundary layer (weight_layer_idx=0, real for LIBERO's structure), so `fuse_res_norm_fp4` must stay off in any full-model config until it's closed -- documented explicitly in both `_single_stream_layer`'s and `_action_single_layer`'s own code comments. |
| 4 | use the existing bit-exact vectorized activation quantizer (`quantize_fp4_sfa_vec.cu`, used by Pi0.5) instead of the scalar one | 0 | 2-4 ms (inf) | **done and wired** (`flash_rt/executors/fp4_utils.py:quant_act_nvfp4`, tries `quantize_fp4_dynamic_sfa_fp16_vec` first, falls back to the scalar kernel on rc != 0; CPU-only fallback-order test `tests/test_fp4_utils_quant_act.py`, 4/4 passed -- the extension is not built on this machine, so the vec kernel's own numerics were not re-run here, only its call-order contract). THOR_CHECKLIST.md X0. |
...
| 6 | step-boundary kernel: fused Euler-step + next-step fp32->fp16 cast (head/action_encoder GEMM bias-epilogue fusion evaluated separately, see below) | -9 (not -70: 9 of 10 Euler/cast pairs fuse, the last step has no following cast) | **measured 0.03-0.09 ms total, not 0.5-1 ms** (local Ada, eager wall-clock delta 3-5.5 us/pair, CUDA-event device-only delta 10.4 us/pair, x9; a clean CUDA-graph-replay comparison was not obtained -- the raw `fvk` bindings' literal `stream=0` argument bypassed torch's capture context, a measurement-harness gap, not a correctness one; graph replay would likely shrink CPU-side launch overhead further, so this is probably an upper bound) | **kernel authored and bit-exact** (`csrc/kernels/fused_step_boundary/euler_step_cast_fused.cu`, `tests/test_imagewam_step_boundary_kernel.py`, 3 seeds x 8 cases including near-zero/negative velocity and delta, real dims num_action=64 action_dim=7). The head/action_encoder bias-epilogue half of this candidate (originally scoped in) is a separate, NOT recommended sub-item: no `fp16_nn_bias` exists in `GemmRunner` (only `bf16_nn_bias`/`fp8_nn_bias*`); a line-for-line FP16 adaptation fails on this dev machine at `cublasLtMatmulAlgoGetHeuristic` (status 15), and the existing production `bf16_nn_bias` ALSO fails here (links a stale system `libcublasLt.so.11`) -- both failures are labelled machine-specific per the project's own rule (issues.md/no-local-only-closure), untested on Thor, ceiling is one launch removed once per step (10x, K=7 GEMM, near-instant already). Roadmap estimate revised down; this whole candidate is now a small, safe, low-priority item, not a 0.5-1 ms one. |
...
| 8 | last backbone single block computes only the K/V it exports | -12 nodes, -52 MB weights | 2-3 ms (inf) | **design proven bit-exact** (`docs/imagewam_last_block_kv_only.md`, `tests/test_imagewam_last_block_kv_only.py`: at real production shape, `torch.equal` True, not just cosine-close -- smaller synthetic shapes only agree to a few ULP, an ordinary cuBLASLt reduction-order non-associativity at the narrower N, not a bug). Confirmed first, not assumed: the ActionDiT/backbone K/V relation is a per-layer BIJECTION (`attn_backend.py:get_slot_ptrs`, one backbone layer's K/V per matching ActionDiT layer index, not all-layers-read-by-every-step), so this applies to exactly the LAST single-stream backbone layer (site_layer_idx=24) and no other -- every earlier layer's residual write feeds the next layer and must run in full. Only K needs RMSNorm+RoPE (V never does, in the real code either way). Needs only a one-time contiguous column slice of the real `linear1.weight` (same pattern `CutlassFp16SwiGluMlp` already uses), no new kernel: `ada_layer_norm_bf16in_fp16out` / `gemm.fp16_nn` at narrower N / `rms_norm_fp16` / `rope_apply_fp16_perhead` unchanged, called on a slice. Precise counted removal at real dims: 58,392,576 bytes (55.7 MiB, nvfp4) of weights, ~198 GFLOP, 9 of 15 `_single_stream_layer` call sites (the backbone's own self-attention at that layer, `attn_out_proj`, `mlp_down`, `_add_inplace`, `gate_res_bf16res`, plus Q's split/norm/rope). One-time per prefill call, not per denoise step. **Wired** (plan.md Phase 3): new function `_single_stream_layer_kv_only`, `checkpoint_loader.py`'s `_extract_single_block(kv_only=True)` adds `"linear1_kv.weight"` for the last backbone single layer only, `imagewam_thor.py`'s generic weight-wrap loop needed no change, `dims["last_layer_kv_only"]` opt-in. A real bug (missing `rope_apply_fp16_perhead` on K, caught by the new wiring test) found and fixed while verifying. `torch.equal` locally against the full, unmodified `_single_stream_layer`'s own K/V at real production shape (`tests/test_imagewam_thor_real_wiring.py::test_single_stream_kv_only_matches_full_block_at_real_shape`, stable over 3 runs). **Thor-confirmed** (`0922h`, HEAD `11e8869`, no rebuild needed): `torch.equal`/`bit_exact=True`, `max_abs=0` for both K and V. Phase 3 closed; AWQ interaction with this flag remains unverified end-to-end. |
...
If 1 and 3 land (the two real per-step wins; 6 is now known small and 2 needs Thor), a step's node count drops by roughly 1670+340 over the whole denoise loop (about 40% of the loop's ~4280 nodes), closer to the HyVLA-precedented "fewer, fatter small kernels" bet than to launch-count cuts alone (6's measured 0.03-0.09 ms confirms that a pure launch-count cut is small on this hardware when it does not also remove a DRAM pass, matching the HyVLA caveat directly rather than as inference now).
...
- The kernel profile (THOR_CHECKLIST.md X4) ran (`0921x`): nvfp4 at 24 valid tokens, GEMM 77.24 ms and `rms_norm` 23.0 ms x560 are the largest contributors of the replay's 123.51 ms, GEMM well below achievable TFLOPs, kernels <10us only 13% of the time -- consistent with 1 (removes the separate `rms_norm` launches) and the GEMM-efficiency finding of ISSUE-088 mattering more than pure launch count. The stage x operator grid (X5) did not run yet -- all four attempts returned zero kernels (a missing CUPTI warm-up, now fixed alongside a `flash`/`flash_rt`-namespace misclassification bug); rerun needed for the per-stage GEMM TFLOPs/GB/s breakdown.
- Wiring: candidates 1, 3, 4, 6, 8 have standalone kernels/designs ready; 4 is wired (quant_act_nvfp4); 1, 3, 6, 8 are not wired into `pipeline_thor.py`, `csrc/bindings.cpp` or `CMakeLists.txt`. Wiring is inherently serial (1, 3, 8 all touch the same per-layer functions in `pipeline_thor.py`), one owner, one fusion at a time, each gated by `imagewam_fusion_ab.py`-style A/B, following OPT-015/016/017's own precedent (and each behind its own `dims` flag so it stays toggleable, matching `merge_qkv_mlp`/`merge_linear2`/`fuse_res_norm`).
- 3 is confirmed bit-exact against TODAY's `gate_res_ada_layer_norm_bf16res`/`_fp16` on Thor (`0921x`, X6, 16/16) -- the earlier "very likely, unconfirmed" caveat is resolved; safe to wire.
- Once wired: Thor rebuild (`ENABLE_SM100_CUTLASS`), the existing bit-exact A/B gates, then X4/X5 for the measured kernel-count and latency win.
- 2 (FA4 direct write) has no local validation path at all; needs Thor from the start.
- 6's bias-epilogue half: not recommended (see table); the Euler+cast half is safe but small -- low priority for wiring effort relative to 1 and 3.
...
- `0921` (commit `de0ef51`, Jetson AGX Thor, MAXN, GPC 1.575 GHz / NVD 1.692 GHz, `emc_locked=null`, GPU idle, raw logs under `/home/jingwu/thor_val/0921/`, nvfp4, libero_spatial): the served default is now trim + FA4 at both sites + the native VAE inside the graph (`plan.md` phase F1). Gate against fixture v2, three repeats: 105.42 / 105.93 / 105.66 ms (spread 0.51 ms), vs official median 0.99931 / 0.99932 / 0.99933 (min 0.99884-0.99896), `effective_config ... use_fa4=True use_fa4_mot=True fa4_fallback_reason=None` each time, seeded as `served_default` in `latency_baselines.json`; the untrimmed reference gate still passes at 202.38 ms. Flag rows (P50 ms, vs official median): `default` 202.2 (0.99764), `vae` 190.2, `vae_trim` 103.3 (0.99932), `vae_trim_fa4bb` 98.9, `stack` 94.4 (0.99931), `stack_no_vae` 105.3, `stack_no_trim` 131.2 (0.99750); profile rows `default` 93.1, `fast` 93.3, `native` 115.0; `FLASHRT_THOR_FA4=0` 103.2 with no error. FA4 first-use compile: cold 14.67 s against 11.79 s warm construction (a one-time 2.89 s). Target workload (3x256x256) with the new default: `infer()` 121.92 ms, ABI 123.64 ms (native skipped: in-graph VAE); the `native` profile there: 180.01 / 140.26 / 139.97 ms. `fp8_static_cutlass` `stack` row with the re-recorded version-3 calibration: vs official median 0.99994 (min 0.99989), 104.7 ms. The gate's P50 (105.4) and the end-to-end `stack` row (94.4) describe one configuration with different prompts (gate: the fixture's last observation), an inference not separately verified; the two columns are not comparable.

...
graph-free single-measured-run methodology, so all three precisions are
directly comparable. This one uses plain fp16_nn for every projection
(no quantization at all) -- fully tested on this dev machine (Ada
sm_89), unlike the FP4/FP8 variants which need Blackwell hardware or a
cuBLASLt FP8 configuration this dev venv's cuBLASLt (12.8.04) does not
support (see imagewam_gemm_precision_compare.py's own findings).

Real weight memory footprint (this pipeline's own reduced-K/V-width
convention, not the checkpoint-shaped WEIGHT_SPEC total) is ~5.56GB at
FP16 -- smaller than the ~6.95GB figure quoted for the checkpoint-
...
class _Fp16Linear:
    """out[M,N] (fp16) = x[M,K] (fp16) @ W[K,N] (fp16), plain cuBLASLt GEMM.

    Shared GemmRunner instance across all layers (constructing one per
    call/layer would allocate a 256MB workspace repeatedly inside what
    should be a steady-state loop -- see plan.md Phase 3's own
    documented fix for exactly this mistake).
    """

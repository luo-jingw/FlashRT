...
the real weight + a representative activation, during the warmup
phase) before falling back to plain `fp16_nn` for the timed loop.
`autotune_fp16_nn` real-benchmarks up to `num_algos` cuBLASLt candidate
algorithms and mutates the SAME cached entry `fp16_nn` reads from
(confirmed by reading csrc/gemm/gemm_runner.cu directly: `entry.algo =
heuristics[best_idx].algo` writes into the identical `CachedGemm&`
`get_or_create_cached` returns to both functions) -- this can only
match or beat the default heuristic's top-1 pick, never regress
correctness (same math, different algorithm selection).
"""
...
class _Fp16Linear:
    """out[M,N] (fp16) = x[M,K] (fp16) @ W[K,N] (fp16), autotuned cuBLASLt GEMM.

    Autotunes once per instance, lazily, on the first real call (using
    the actual x/out buffers/shape passed in) -- naturally falls inside
    this benchmark's own warmup phase, matching every other linear op's
    own lazy-scratch-allocation convention.
    """
    _gemm = fvk.GemmRunner()

    def __init__(self, n: int, k: int):
        self.n, self.k = n, k
        self.w = torch.randn(k, n, dtype=FP16, device=DEV)
        _keepalive.append(self.w)
        self._tuned = False

    def __call__(self, x: torch.Tensor, out: torch.Tensor, m: int, stream: int = 0):
        if not self._tuned:
            _Fp16Linear._gemm.autotune_fp16_nn(x.data_ptr(), self.w.data_ptr(), out.data_ptr(),
                                                m, self.n, self.k, 16)
            self._tuned = True
        _Fp16Linear._gemm.fp16_nn(x.data_ptr(), self.w.data_ptr(), out.data_ptr(), m, self.n, self.k, stream)
...
class FullImageWAMFP16Autotuned:
    """All 25 backbone layers + all 25 ActionDiT layers, plain FP16 GEMMs.
    ...
    def __init__(self):
        ...
        for _ in range(NUM_DOUBLE):
            self.double_layers.append(dict(
                txt_in=_Fp16Linear(HIDDEN, JOINT_ATTN_DIM),
                txt_q=_Fp16Linear(HIDDEN, HIDDEN), txt_k=_Fp16Linear(HD, HIDDEN),
                txt_v=_Fp16Linear(HD, HIDDEN), txt_proj=_Fp16Linear(HIDDEN, HIDDEN),
                txt_mlp0=_Fp16Linear(MLP_HIDDEN, HIDDEN), txt_mlp2=_Fp16Linear(HIDDEN, MLP_HIDDEN),
                img_q=_Fp16Linear(HIDDEN, HIDDEN), img_k=_Fp16Linear(HD, HIDDEN),
                img_v=_Fp16Linear(HD, HIDDEN), img_proj=_Fp16Linear(HIDDEN, HIDDEN),
                img_mlp0=_Fp16Linear(MLP_HIDDEN, HIDDEN), img_mlp2=_Fp16Linear(HIDDEN, MLP_HIDDEN),
            ))
        self.single_layers = []
        for _ in range(NUM_SINGLE):
            self.single_layers.append(dict(
                q=_Fp16Linear(HIDDEN, HIDDEN), k=_Fp16Linear(HD, HIDDEN), v=_Fp16Linear(HD, HIDDEN),
                mlp_in=_Fp16Linear(MLP_HIDDEN, HIDDEN),
                attn_out_proj=_Fp16Linear(HIDDEN, HIDDEN), mlp_down=_Fp16Linear(HIDDEN, MLP_HIDDEN),
            ))
    ...
    # _action_double_layer/_action_single_layer exactly, plain FP16 GEMMs ----
    ...
    def _init_action_layers(self):
        ...
        for _ in range(NUM_DOUBLE):
            self.action_double_layers.append(dict(
                q=_Fp16Linear(ACTION_ATTN_WIDTH, ACTION_HIDDEN_DIM),
                k=_Fp16Linear(HD, ACTION_HIDDEN_DIM), v=_Fp16Linear(HD, ACTION_HIDDEN_DIM),
                proj=_Fp16Linear(ACTION_HIDDEN_DIM, ACTION_ATTN_WIDTH),
                mlp0=_Fp16Linear(ACTION_MLP_HIDDEN, ACTION_HIDDEN_DIM),
                mlp2=_Fp16Linear(ACTION_HIDDEN_DIM, ACTION_MLP_HIDDEN),
            ))
        ...
        for _ in range(NUM_SINGLE):
            self.action_single_layers.append(dict(
                q=_Fp16Linear(ACTION_ATTN_WIDTH, ACTION_HIDDEN_DIM),
                k=_Fp16Linear(HD, ACTION_HIDDEN_DIM), v=_Fp16Linear(HD, ACTION_HIDDEN_DIM),
                mlp_in=_Fp16Linear(ACTION_MLP_HIDDEN, ACTION_HIDDEN_DIM),
                attn_out_proj=_Fp16Linear(ACTION_HIDDEN_DIM, ACTION_ATTN_WIDTH),
                mlp_down=_Fp16Linear(ACTION_HIDDEN_DIM, ACTION_MLP_HIDDEN),
            ))
...
def main():
    ...
    print(f"Building full {NUM_DOUBLE + NUM_SINGLE}-layer FP16 backbone + "
          f"{NUM_DOUBLE + NUM_SINGLE}-layer FP16 ActionDiT "
    ...
    print(f"backbone_prefill_fp16_autotuned (25 layers)    P50={p50:8.3f} ms  P90={p90:8.3f} ms  mean={mean:8.3f} ms")

    p50d, p90d, meand = _time_ms(lambda: model.run_denoise_step(1.0 / num_denoise_steps, 0))
    print(f"one_denoise_step_fp16_autotuned (25 layers)    P50={p50d:8.3f} ms  P90={p90d:8.3f} ms  mean={meand:8.3f} ms")

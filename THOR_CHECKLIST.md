...
0921 一轮（默认提升为最快配置、延迟基线按配置、标定 identity 带相机几何）的待测项 N0–N5 已全部做完并按"用法"第 2 条删除，结论在下面表格指定的文件里（`THOR_STATUS_SUMMARY.md` 的 `0921` 小节）。**本清单目前只有下面一项待测：ISSUE-087 的探针。** 最终结果表（官方 torch 对 FlashRT fp16 / fp8 / fp4 / int8 / int4，LIBERO 与 RoboTwin 两套标准配置）的测量清单等 RoboTwin 配置确定后再加，方案在 `plan.md` 的 "Plan: the final result tables"。
